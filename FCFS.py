...
# Function to simulate teller service for FCFS
def teller_fcfs(teller_id):
    while True:
        customer_id, arrival_time = customer_queue.get()
...
# Function to simulate customer arrivals for FCFS
def customer_arrival_fcfs():
    customer_id = 1
    while True:
        arrival_time = time.time()  # Record arrival time
        with print_lock:
            print(f"Customer {customer_id} enters the Queue at time {arrival_time}")
        customer_queue.put((customer_id, arrival_time))
        customer_id += 1
        arrival_interval = random.randint(1, 3)
        time.sleep(arrival_interval)

# Function to run FCFS simulation and calculate metrics
def run_fcfs_simulation():
    # Create threads for tellers
    teller_threads = []
    for i in range(num_tellers):
        t = threading.Thread(target=teller_fcfs, args=(i,))
        t.start()
        teller_threads.append(t)

    # Create a thread for customer arrivals
    customer_thread = threading.Thread(target=customer_arrival_fcfs)
